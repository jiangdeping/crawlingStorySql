...
#https://blog.csdn.net/weixin_43935187/article/details/85763356?utm_medium=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-5.channel_param&depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-5.channel_param
...
class MySQL(object):
    ...
    def __init__(self):
        ...
        self.cur = None
    def connect(self):
        db_conf = ConfigParse.get_db_config()
        ...
        # self.charset = "utf8",
        # self.use_unicode = True,
        self.conn=MySQLdb.connect(host=db_conf["host"],port=int(db_conf["port"]),user=db_conf["user"],
                                  passwd=db_conf["password"],db=db_conf["db"]
    ...
    def inertStoryUrl(self, storyNo, storyTitle, storyUrl):  # 插入小说的链接地址
        sql = "INSERT INTO story_url(storyno,storytitle,storyurl) VALUES ('{}','{}','{}')".format(storyNo, storyTitle,
                                                                                                  storyUrl)
        self.cur.execute(sql)
        self.conn.commit()
    ...
    def insetStoryContentUrl(self, storyno, storychapter, chapter_num_url):
        sql = "INSERT INTO story_content_url(storyno,chapter_num,chapter_num_url) VALUES ('{}','{}','{}')".format(
            storyno, storychapter, chapter_num_url)
        self.cur.execute(sql)
        self.conn.commit()
    ...
    def insertStory(self, url, text, storyno="82919"):
    ...
    def getSrotyDownUrl111(self,storyno):  # 获取需要下载小说的链接地址,num为下载故事的个数
        self.connect()
        sql = "SELECT storyurl FROM story_url LIMIT %s" % storyno
        print(sql)
    ...
    def insertStory3(self,url, text, storyno="82919"):
        conn=self.connect()
        storytitle = "123"
        reg = re.compile(r'http://m.xsqishu.com(.+).html')
        identical = reg.findall(url)  # 同一小说相同的部分
        ...
        conn.close()
# db = MySQL()
# db.getSrotyDownUrl111("82919")
# print(db.getSrotyDownUrl111("82919"))
